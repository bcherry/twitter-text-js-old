var TwitterText = (function () { 
    ...
    regexes['auto_link_usernames_or_lists'] = /([^a-zA-Z0-9_]|^)([@＠]+)([a-zA-Z0-9_]{1,20})(\/[a-zA-Z][a-zA-Z0-9\x80-\xff\-]{0,79})?/g;
    regexes['auto_link_emoticon'] = /(8\-\#|8\-E|\+\-\(|\`\@|\`O|\&lt;\|:~\(|\}:o\{|:\-\[|\&gt;o\&lt;|X\-\/|\[:-\]\-I\-|\/\/\/\/Ö\\\\\\\\|\(\|:\|\/\)|∑:\*\)|\( \| \))/g;
	...
	my.hit_highlight = function (text, hits, options) {
		...
		var tag_name = options.tag || default_highlight_tag,
			...
			first_splits = text.split("<"), second_splits,
			chunks = [],
			split,
			i,
			j;
		
		for (i = 0; i < first_splits.length; i += 1) {
			split = first_splits[i];
			if (!split) {
				chunks.push("");
			} else {
				second_splits = split.split(">");
				for (j = 0; j < second_splits.length; j += 1) {
		...
		for (index = 0; index < flat_hits.length; index += 1) {
			hit = flat_hits[index];
			tag = tags[index % 2];
			placed = false;
			
			while (chunk != null && hit >= prev_chunks_len + chunk.length) {
		...
		if (chunk != null) {
			...
			for (index = chunk_index + 1; index < chunks.length; index += 1) {
