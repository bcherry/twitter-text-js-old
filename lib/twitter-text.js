var TwitterText = (function () { 
	...
	var default_highlight_tag = "em";

	my.hit_highlight = function (text, hits, options) {
		hits = hits || [];
		options = options || {};
		...
		var tag_name = options.tag || default_highlight_tag;
		var tags = ["<" + tag_name + ">", "</" + tag_name + ">"];
		var first_splits = text.split("<"), second_splits;
		var chunks = [];
		var split;
		for (var i = 0; i < first_splits.length; i += 1) {
			split = first_splits[i];
			if (!split) {
				chunks.push("");
			} else {
				second_splits = split.split(">");
				for (var j = 0; j < second_splits.length; j += 1) {
					chunks.push(second_splits[j]);
				}
			}
		}
		var result = "";
		var chunk_index = 0,
			chunk = chunks[0];
		var prev_chunks_len = 0;
		var chunk_cursor = 0;
		var start_in_chunk = false;
		var chunk_chars = chunk; // TODO: below
      // chunk_chars = chunk.respond_to?("mb_chars") ? chunk.mb_chars : chunk.respond_to?("chars") && chunk.chars.respond_to?("[]") ? chunk.chars : chunk
		console.log("chunk is %o", chunk);
		var flat_hits = [];
		for (var i = 0; i < hits.length; i += 1) {
			for (var j = 0; j < hits[i].length; j += 1) {
				flat_hits.push(hits[i][j]);
			}
		}
		for (var index = 0; index < flat_hits.length; index += 1) {
			var hit = flat_hits[index];
			var tag = tags[index % 2];
			var placed = false;
			while (chunk && !(hit < prev_chunks_len + chunk.length)) {
				result += chunk_chars.slice(chunk_cursor);
				...
				chunk_chars = chunk; // TODO
				start_in_chunk = false;
			}
			
			if (!placed && chunk) {
				var hit_spot = hit - prev_chunks_len;
				result += chunk_chars.slice(chunk_cursor, hit_spot) + tag;
		...
		if (chunk) {
			...
			for (var index = chunk_index + 1; index < chunks.length; index += 1) {
